...
class UserSerializer(serializers.ModelSerializer):
    ...
    role is a computed field so is_staff / is_superuser users
    always get 'admin' regardless of what's stored in the role column.
    """
    profile_picture = serializers.ImageField(read_only=True)
    # Override role as a method field so we can normalise it
    ...
    def get_role(self, obj):
        """Return 'admin' for any staff/superuser regardless of role field value."""
        if obj.is_staff or obj.is_superuser or obj.role == 'admin':
            return 'admin'
        return obj.role
...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    Supports text fields and image upload.
    """
